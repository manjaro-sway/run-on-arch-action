...
run_container () {
  ...
  source "$(dirname "$0")/run-on-arch-setup.sh"
  ...
  chmod +x "$(dirname "$0")/run-on-arch-commands.sh"
  ...
  docker run \
    ...
    -v "$(dirname "$0"):$(dirname "$0")" \
    --tty \
    ${DOCKER_RUN_ARGS[@]} \
    "${CONTAINER_NAME}:latest" \
    "$(dirname "$0")/run-on-arch-commands.sh"
}
